...
public class PCBuilderController implements Initializable {
    ...
    private void initPSU() throws FileNotFoundException {
        ...
        for (int i = 0; i < allPSU.size(); i++) {
            String[] specs = allPSU.get(i).split(",");
            partsList.getItems().add(specs[0]);
        }
    }
    ...
    private double getComponentCost(String selected, int Stage) {
        double partCost = 0;
        ArrayList[] arrays = new ArrayList[]{allCPU, allMobo, allRAM, allCase, allGPU, allPSU};
        if (stage < 6) {
    ...
    private void getPSUCompatibility() {
        if (stage == 4) {
            String target;
            for (int i = 0; i < allGPU.size(); i++) {
                if (allGPU.get(i).contains(current)) {
                    target = allGPU.get(i);
    ...
    private void setSpecsList(String selected) {
        ArrayList[] arrays = new ArrayList[]{allCPU, allMobo, allRAM, allCase, allGPU, allPSU};
        String[][] strings = new String[][]{cpuSpecsLabel, moboSpecsLabel, ramSpecsLabel, casesSpecsLabel, gpuSpecsLabel, psuSpecsLabel};
        specsList.getItems().clear();
